//*****************************************************************************************/
// To compile run: gcc -g Assignment2.c -pthread -lrt -o Assignment2
/*****************************************************************************************/
#include <time.h>
#include <stdio.h>
#include <string.h>
...
{
  printf("inside init thread args\n");
...
{
  printf("inside CreateAndArmTimer function for thread: %d\n", info->threadCount);

  int ret, seconds, nanoseconds;
...
{
  int returnVal;
  returnVal = sigwait(&info->alarm_sig, &info->sig);
  if(returnVal != 0){
    handle_error_en(returnVal, "sigwait");
  }
...
void* threadFunction(void *arg)
{
  ...
  if(tms.tv_nsec % 1000 >= 500 ) myThreadArg->timeStamp[0]++;
	...
	//In a loop call "wait_period(myThreadArg);" taking a timestamp before and after using "clock_gettime(CLOCK_REALTIME, &tms);"
...
int main (int argc, char *argv[])
{
  ...
  // block all real-time signals by using “sigemptyset” and “sigaddset” (add all signals
  // between SIGRTMIN and SIGRTMAX) to create a signal set and then call “pthread_sigmask” with SIG_BLOCK
   sigset_t timer_signal;
   sigemptyset(&timer_signal);
   for (int i = SIGRTMIN; i <=SIGRTMAX; i++)
      sigaddset(&timer_signal, i);
   pthread_sigmask(SIG_BLOCK, &timer_signal, NULL);
